...
| Cloud AI/Whisper | `AI_PROVIDER` plus the Gemini or OpenRouter key/model variables for Go scripts/assistant and retained worker highlights. OpenRouter model values are unrestricted text/chat model slugs. Choose/provision the worker's Whisper model/device/compute type and model cache. |
