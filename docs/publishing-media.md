...
`frontend/scripts/test-publishing-media-browser.mjs` runs against a local frontend and intercepts every API request with a synthetic account. It verifies desktop mouse dragging, touch dragging at 390 px, multi-selection, incremental additions, retained successes on partial failure, retry of the same file, first-item selection, arrow controls, removal, the total count limit, unchanged multipart bytes, saved order, and loaded previews after reopening. It produced no browser runtime errors. Set `SNEEPCUT_BROWSER_URL` to the local frontend (including `/ro` for Romanian) and, if needed, `PLAYWRIGHT_MODULE` to an installed Playwright module. This browser fixture mocks HTTP; the Go suite below separately verifies real database and upload handlers.
