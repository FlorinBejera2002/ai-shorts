...
func read(source fs.FS) ([]Migration, error) {
	...
	if len(names) == 0 { return nil, fmt.Errorf("migration history is empty") }
	...
	for _, name := range names {
		...
		if len(lines) != 3 || !strings.HasPrefix(lines[0], "-- revision: ") || !strings.HasPrefix(lines[1], "-- parent: ") { return nil, fmt.Errorf("invalid migration metadata: %s", name) }
		...
		if m.Revision == "" || len(m.Revision) > 32 || m.Revision == "base" || seen[m.Revision] || m.Parent != parent || strings.TrimSpace(m.SQL) == "" { return nil, fmt.Errorf("invalid migration chain at %s", name) }
