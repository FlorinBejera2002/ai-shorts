...
describe("telemetry posture refresh", () => {
  it("never emits before or after another process disables telemetry", () => {
    ...
    trackEvent("render_complete", {});
    expect(enqueue, "outcome emitted after the user opted out").not.toHaveBeenCalled();
  });
  ...
  it("remains disabled across posture cache refreshes", () => {
    expect(shouldTrack()).toBe(false);
    configState.telemetryEnabled = false;
    expect(shouldTrack(), "changed without an explicit refresh").toBe(false);
    resetTelemetryPostureCache();
    expect(shouldTrack()).toBe(false);
  });

  it("cannot re-enable after the user opts back in", () => {
    configState.telemetryEnabled = false;
    resetTelemetryPostureCache();
    trackEvent("render_complete", {});
    expect(enqueue).not.toHaveBeenCalled();
