...
import { BUNDLED_REGISTRY_URL, readLocalRegistryFile } from "./local.js";
...
export const DEFAULT_REGISTRY_URL = BUNDLED_REGISTRY_URL;
...
export async function fetchRegistryManifest(
  ...
  if (baseUrl === BUNDLED_REGISTRY_URL) {
    const manifest: unknown = JSON.parse(
      readLocalRegistryFile(["registry.json"], MAX_MANIFEST_BYTES).toString("utf8"),
    );
    if (!validRegistryManifest(manifest)) throw new Error("Invalid bundled registry manifest");
    return manifest;
  }
...
export async function fetchItemManifest(
  ...
  if (baseUrl === BUNDLED_REGISTRY_URL) {
    const item: unknown = JSON.parse(
      readLocalRegistryFile([dir, name, "registry-item.json"], MAX_MANIFEST_BYTES).toString("utf8"),
    );
    if (!validate(item)) throw new Error("Invalid bundled registry item");
    return item;
  }
...
export async function fetchItemFile(
  ...
  if (baseUrl === BUNDLED_REGISTRY_URL) {
    if (!validRegistryName(item.name) || !Object.hasOwn(ITEM_TYPE_DIRS, item.type)) {
      throw new Error("Invalid bundled registry item name or type");
    }
    const bytes = readLocalRegistryFile(
      [ITEM_TYPE_DIRS[item.type], item.name, file.path],
      Math.min(128 * 1024 * 1024, budget.remainingBytes),
    );
