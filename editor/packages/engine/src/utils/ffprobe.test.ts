...
import { basename, resolve, toNamespacedPath } from "path";
...
describe("ffprobe option separator", () => {
  ...
  it("uses an extended-length Windows path for deeply nested render outputs", async () => {
    const { spawn, calls } = createSpawnSpy([
      {
        kind: "exit",
        code: 0,
        stdout: JSON.stringify({
          streams: [
            { codec_type: "video", codec_name: "h264", width: 16, height: 16, r_frame_rate: "1/1" },
          ],
          format: { duration: "1" },
        }),
      },
    ...
    vi.doMock("child_process", () => ({ spawn }));
    const { extractMediaMetadata } = await import("./ffprobe.js");
    const filePath = resolve("synthetic", "nested".repeat(50), "render.mp4");
    await extractMediaMetadata(filePath);
    expect(calls[0]?.args.slice(-2)).toEqual([
      "--",
      process.platform === "win32" ? toNamespacedPath(filePath) : filePath,
    ]);
  });
});
