...
describe("registerThumbnailRoutes", () => {
  it("rejects encoded traversal to another project's composition before generation", async () => {
    const adapter = createAdapter();
    const other = createAdapter();
    const outsideProject = await other.resolveProject("other");
    if (!outsideProject) throw new Error("missing project");
    await writeComposition(other, 111, 222);
    const app = new Hono();
    registerThumbnailRoutes(app, adapter);

    const path = encodeURIComponent(`../${basename(outsideProject.dir)}/index.html`);
    const response = await app.request(`http://localhost/projects/demo/thumbnail/${path}`);
  ...
  it("rejects malformed encoded composition paths", async () => {
    const adapter = createAdapter();
    const app = new Hono();
    registerThumbnailRoutes(app, adapter);
    const response = await app.request("http://localhost/projects/demo/thumbnail/bad%ZZ.html");
    expect(response.status).toBe(400);
    expect(adapter.generateThumbnail).not.toHaveBeenCalled();
  });
