...
export function ClipCard({
  ...
  return (
    <Link
      ...
      className="panel group relative flex min-w-0 flex-col overflow-hidden p-0 transition-all hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-lg animate-slide-up"
      style={{ animationDelay: `${Math.min(index, 10) * 35}ms` }}
      aria-label={`${labels.open}: ${clip.title}`}
    >
      <div className="relative aspect-video overflow-hidden bg-[radial-gradient(circle_at_20%_20%,hsl(var(--primary)/.18),transparent_55%),hsl(var(--muted))]">
        {clip.thumbnailUrl ? (
          <img
