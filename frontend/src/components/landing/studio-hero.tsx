...
function StageCanvas() {
  ...
  const spots = useRef([
    { origin: 'top', x: 0.06, color: [168, 85, 247], phase: 0 },
    { origin: 'top', x: 0.21, color: [192, 38, 211], phase: 2 },
    { origin: 'top', x: 0.35, color: [59, 130, 246], phase: 0.5 },
    { origin: 'top', x: 0.5, color: [139, 92, 246], phase: 0.8 },
    { origin: 'top', x: 0.65, color: [217, 70, 239], phase: 4 },
    { origin: 'top', x: 0.79, color: [37, 99, 235], phase: 3 },
    { origin: 'top', x: 0.94, color: [99, 102, 241], phase: 1.2 }
  ])
  ...
  const draw = useCallback((time: number) => {
    ...
    skyGrad.addColorStop(0, '#030010')
    skyGrad.addColorStop(0.15, '#0a0320')
    skyGrad.addColorStop(0.35, '#0e0428')
    skyGrad.addColorStop(0.5, '#0d0322')
    skyGrad.addColorStop(0.7, '#060114')
    ctx.fillStyle = skyGrad
    ctx.fillRect(0, 0, w, h)

    // ── Nebula / cosmic glow ──
    const drawNebula = (cx: number, cy: number, rx: number, ry: number, r: number, g: number, b: number, alpha: number) => {
      ctx.save()
      ctx.translate(cx, cy)
      ctx.scale(1, ry / rx)
    ...
    drawNebula(w * 0.3, h * 0.15, w * 0.35, w * 0.2, 100, 40, 180, 0.12 + Math.sin(t * 0.2) * 0.03)
    drawNebula(w * 0.7, h * 0.1, w * 0.3, w * 0.18, 40, 60, 200, 0.1 + Math.sin(t * 0.25 + 1) * 0.025)
    drawNebula(w * 0.5, h * 0.25, w * 0.25, w * 0.15, 160, 50, 220, 0.08 + Math.sin(t * 0.3 + 2) * 0.02)
    drawNebula(w * 0.15, h * 0.3, w * 0.2, w * 0.12, 20, 80, 180, 0.06)
    drawNebula(w * 0.85, h * 0.25, w * 0.2, w * 0.1, 180, 30, 160, 0.07)
    ...
    for (const s of stars.current) {
      ...
      if (s.size > 1) {
        ...
        ctx.fillStyle = `rgba(180,170,255,${twinkle * 0.1})`
        ctx.fill()
      }
    }
    ...
    ctx.save()
    ctx.beginPath()
    ctx.ellipse(stageCx, stageCy + stageThickness, stageRx, stageRy, 0, 0, Math.PI)
    ...
    const sideGrad = ctx.createLinearGradient(stageCx - stageRx, 0, stageCx + stageRx, 0)
    sideGrad.addColorStop(0, '#030303')
    sideGrad.addColorStop(0.15, '#080707')
    sideGrad.addColorStop(0.35, '#0d0b0a')
    sideGrad.addColorStop(0.5, '#100e0c')
    sideGrad.addColorStop(0.65, '#0d0b0a')
    sideGrad.addColorStop(0.85, '#080707')
    sideGrad.addColorStop(1, '#030303')
    ctx.fillStyle = sideGrad
    ctx.fill()

    // Vertical shading on the side for depth
    const sideVGrad = ctx.createLinearGradient(0, stageCy, 0, stageCy + stageThickness)
    ...
    ctx.save()
    ctx.beginPath()
    ctx.ellipse(stageCx, stageCy + stageThickness, stageRx, stageRy, 0, 0.05, Math.PI - 0.05)
    const rimGrad = ctx.createLinearGradient(stageCx - stageRx, 0, stageCx + stageRx, 0)
    rimGrad.addColorStop(0, 'transparent')
    rimGrad.addColorStop(0.15, 'rgba(168,85,247,0.3)')
    rimGrad.addColorStop(0.35, 'rgba(59,130,246,0.15)')
    rimGrad.addColorStop(0.5, 'rgba(255,255,255,0.1)')
    rimGrad.addColorStop(0.65, 'rgba(6,182,212,0.15)')
    rimGrad.addColorStop(0.85, 'rgba(192,38,211,0.3)')
    ...
    ctx.closePath()
    const topGrad = ctx.createRadialGradient(stageCx, stageCy - stageRy * 0.2, 0, stageCx, stageCy, stageRx)
    topGrad.addColorStop(0, '#141210')
    topGrad.addColorStop(0.35, '#0f0d0b')
    topGrad.addColorStop(0.7, '#0a0908')
    topGrad.addColorStop(1, '#060505')
    ...
    for (let gy = stageCy - stageRy; gy < stageCy + stageRy; gy += 4) {
      ...
      ctx.strokeStyle = `rgba(${60 + (gy % 20)},${40 + (gy % 12)},${30 + (gy % 8)},0.4)`
      ctx.lineWidth = 0.3 + ((gy * 3) % 2) * 0.3
      ctx.beginPath()
      const rowHalf = Math.sqrt(Math.max(0, 1 - Math.pow(frac * 2 - 1, 2))) * stageRx
    ...
    for (let ring = 0.3; ring < 1; ring += 0.2) {
      ctx.beginPath()
      ctx.ellipse(stageCx, stageCy, stageRx * ring, stageRy * ring, 0, 0, Math.PI * 2)
      ctx.stroke()
    }
    ctx.globalAlpha = 1
    ctx.restore()
    ...
    const topRimGrad = ctx.createLinearGradient(stageCx - stageRx, 0, stageCx + stageRx, 0)
    topRimGrad.addColorStop(0, 'transparent')
    topRimGrad.addColorStop(0.2, 'rgba(168,85,247,0.25)')
    topRimGrad.addColorStop(0.4, 'rgba(139,92,246,0.15)')
    topRimGrad.addColorStop(0.5, 'rgba(255,255,255,0.1)')
    topRimGrad.addColorStop(0.6, 'rgba(6,182,212,0.15)')
    topRimGrad.addColorStop(0.8, 'rgba(192,38,211,0.25)')
    ...
    for (const [index, spot] of spots.current.entries()) {
      ...
      const sx = fromTop ? (spot.x ?? 0.5) * w : fromLeft ? -w * 0.035 : w * 1.035
      ...
      const targetX = stageCx + (index - (spots.current.length - 1) / 2) * stageRx * 0.075
      const targetY = stageCy - stageRy * 0.08
      const finalAngle = (Math.atan2(targetX - sx, targetY - sy) * 180) / Math.PI
      const topX = spot.x ?? 0.5
      const entryAngle = fromTop ? (topX < 0.5 ? -34 : topX > 0.5 ? 34 : 0) : fromLeft ? -58 : 58
      const introDelay = index * 90
      const introProgress = Math.max(0, Math.min(1, (elapsed - introDelay) / 1900))
      const easedProgress = 1 - (1 - introProgress) ** 4
      const angleDeg = entryAngle + (finalAngle - entryAngle) * easedProgress
      const angleRad = (angleDeg * Math.PI) / 180
      const settledPulse = 0.9 + Math.sin(t * 0.45 + spot.phase) * 0.06
      const pulse = (0.18 + introProgress * 0.82) * settledPulse

      const len = h * 1.25
      const endX = sx + Math.sin(angleRad) * len
      const endY = sy + Math.cos(angleRad) * len
      const distanceFromCenter = Math.abs(index - (spots.current.length - 1) / 2) / 3
    ...
    for (const p of dust.current) {
      ...
      ctx.fillStyle = `rgba(210,195,255,${alpha})`
      ctx.fill()
    }

    // ── Vignette ──
    const vg = ctx.createRadialGradient(w * 0.5, h * 0.42, w * 0.22, w * 0.5, h * 0.42, w * 0.72)
    ...
    >
      ...
      <div className="absolute left-1/2 top-[34%] -translate-x-1/2 -translate-y-1/2 z-[2] pointer-events-none select-none">
        <motion.div
          ...
          animate={{ opacity: 0.06, scale: 1, y: [0, -4, 0] }}
          transition={{
      ...
      <div className="absolute bottom-0 left-0 right-0 h-28 bg-gradient-to-t from-[#060608] to-transparent z-10" />
    </section>
  )
}
