    ...
    <div ref={ref} className="mt-4 space-y-2.5">
      {rows.map((row, i) => (
        <div key={row.score} className="flex items-center gap-3">
          <div className="h-8 w-12 shrink-0 rounded-md border border-[#5139EF]/20 bg-gradient-to-br from-[#5139EF]/25 to-[#5139EF]/10" />
          <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
            <motion.div
              className="h-full rounded-full bg-[#5139EF]"
              initial={{ width: 0 }}
          ...
          <motion.div
            className="flex shrink-0 items-center gap-1 rounded-full bg-[#5139EF]/10 px-2 py-0.5"
            initial={{ opacity: 0, scale: 0.7 }}
            animate={isInView ? { opacity: 1, scale: 1 } : {}}
            transition={{ duration: 0.4, delay: 0.9 + i * 0.15, ease: EASE }}
          >
            <Sparkles className="h-2.5 w-2.5 text-[#5139EF]" />
            <span className="text-[10px] font-bold text-[#5139EF]">
              {row.score}
            </span>
          </motion.div>
        </div>
...
export function ScriptViz() {
  ...
  return (
    ...
    >
      <motion.div
        className="inline-flex items-center rounded-md bg-[#5139EF]/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#5139EF]"
...
export function CropViz() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute top-1/2 h-6 w-6 -translate-y-1/2 rounded-full bg-[#5139EF]/50 blur-[1px]"
        animate={
          isInView
            ? reduceMotion
              ? { left: '35%' }
      ...
      <motion.div
        className="absolute bottom-1 top-1 w-11 rounded-md border-2 border-[#5139EF]/70 bg-[#5139EF]/5"
        animate={
          isInView
            ? reduceMotion
              ? { left: '32%' }
      ...
      >
        <div className="absolute left-1/2 top-1 -translate-x-1/2 rounded-sm bg-[#5139EF] px-1 py-px text-[8px] font-bold leading-none text-white">
          9:16
        </div>
      </motion.div>
    </div>
