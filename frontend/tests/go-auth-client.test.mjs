...
const source = readFileSync(new URL('../src/lib/auth-client.ts', import.meta.url), 'utf8')
const compiled = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 } }).outputText
const { createAuthClient } = await import(`data:text/javascript;base64,${Buffer.from(compiled).toString('base64')}`)
const user = { id: 'fixture-user', email: 'fixture@example.test', name: 'Fixture' }
...
test('startup refresh coalesces concurrent requests and sends cookies plus memory bearer', async () => {
  ...
  const client = createAuthClient(async (path, options) => {
    ...
    if (path.endsWith('/refresh')) { refreshes++; await pending.promise; return success('first') }
    headers.push(options.headers.get('Authorization'))
    return Response.json({ ok: true })
  })
  const requests = [client.apiFetch('/api/jobs'), client.apiFetch('/api/clips'), client.refresh()]
  pending.resolve()
...
test('concurrent expired access responses share one refresh and preserve mutation body', async () => {
  ...
  const client = createAuthClient(async (path, options) => {
    ...
    if (path.endsWith('/refresh')) { refreshes++; return success('new') }
    calls++
    assert.equal(options.method, 'PATCH')
    assert.equal(options.body, '{"title":"updated"}')
    return options.headers.get('Authorization') === 'Bearer old' ? new Response(null, { status: 401 }) : Response.json({ ok: true })
  })
  await client.login(user.email, 'fixture')
  const requests = await Promise.all(Array.from({ length: 8 }, () => client.apiFetch('/api/clips/id', { method: 'PATCH', body: '{"title":"updated"}' })))
...
test('a second unauthorized response stops after one retry and clears auth', async () => {
  ...
  const client = createAuthClient(async path => {
    if (path.endsWith('/login') || path.endsWith('/refresh')) return success('token')
    calls++
...
test('logout fences a pending refresh and revokes its cookie after completion', async () => {
  ...
  const events = []
  const client = createAuthClient(async path => {
    if (path.endsWith('/refresh')) { await pending.promise; events.push('refresh-cookie'); return success('stale') }
...
test('logout failure remains retryable and cannot silently refresh a session', async () => {
  let requests = 0
  const client = createAuthClient(async () => { requests++; return new Response(null, { status: requests === 1 ? 503 : 204 }) })
...
test('service failures do not masquerade as an expired session', async () => {
  const client = createAuthClient(async () => new Response(null, { status: 503 }))
...
test('bearer credentials cannot be sent to an arbitrary URL', async () => {
  let calls = 0
  const client = createAuthClient(async path => { calls++; return path.endsWith('/login') ? success('secret') : Response.json({}) })
  await client.login(user.email, 'fixture')
  await assert.rejects(client.apiFetch('https://attacker.example/api/jobs'), /local/)
...
test('Next contains no database clients, auth authority, or business API handlers', () => {
  ...
  for (const path of walk(root.pathname).filter(path => /\.[jt]sx?$/.test(path))) {
    const text = readFileSync(path, 'utf8')
    assert.doesNotMatch(text, /from ['"](?:next-auth|@prisma|@auth\/prisma|pg['"]|stripe['"]|bcrypt)/, path)
    assert.doesNotMatch(text, /process\.env\.(?:DATABASE_URL|JWT_SECRET|AUTH_SECRET|GOOGLE_CLIENT_SECRET|INTERNAL_API_KEY|STRIPE_SECRET_KEY)/, path)
  }
