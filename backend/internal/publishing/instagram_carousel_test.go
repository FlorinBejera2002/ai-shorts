...
func TestInstagramMixedCarouselWaitsForEveryChildAndPreservesOrder(t *testing.T) {
	...
	job, status, err := p.PublishMedia(ctx, "instagram", "123", creds, media, caption, TikTokOptions{}, InstagramOptions{})
...
func TestInstagramCarouselChildFailureStopsParent(t *testing.T) {
	for _, failure := range []string{"ERROR", "EXPIRED"} {
		t.Run(failure, func(t *testing.T) {
			...
			job, _, err := p.PublishMedia(context.Background(), "instagram", "123", Credentials{}, []PublishMedia{{Type: "image", URL: "https://media.example/a.jpg"}, {Type: "video", URL: "https://media.example/b.mp4"}}, "", TikTokOptions{}, InstagramOptions{})
			if err != nil {
				t.Fatal(err)
			}
...
func TestPublishMediaRejectsUnsupportedCombinationsBeforeNetwork(t *testing.T) {
	...
	for _, sample := range []struct {
		...
		_, status, err := p.PublishMedia(context.Background(), sample.provider, "123", Credentials{}, media, "", TikTokOptions{}, InstagramOptions{})
