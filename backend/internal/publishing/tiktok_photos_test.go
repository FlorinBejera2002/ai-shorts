...
func TestTikTokPhotoPostPreservesAllPhotosAndOptions(t *testing.T) {
	...
	id, status, err := p.PublishMedia(context.Background(), "tiktok", "creator", Credentials{}, photos, "", TikTokOptions{PrivacyLevel: "SELF_ONLY", MusicUsageConfirmed: true, AutoAddMusic: true, PhotoTitle: "Summer"}, InstagramOptions{})
...
func TestTikTokMediaCountsAndVerifiedURLs(t *testing.T) {
	...
	_, status, err := p.PublishMedia(context.Background(), "tiktok", "creator", Credentials{}, []PublishMedia{{Type: "image", URL: "https://media.example/one.jpg"}, {Type: "image", URL: "https://unverified.example/two.jpg"}}, "", TikTokOptions{}, InstagramOptions{})
