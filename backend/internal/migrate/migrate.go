...
func apply(ctx context.Context, db *sql.DB, chain []migrations.Migration) (string, error) {
	...
	defer tx.Rollback()
	if _, err = tx.ExecContext(ctx, "SELECT pg_advisory_xact_lock(7364338762401)"); err != nil {
		return "", fmt.Errorf("lock migrations: %w", err)
	}
	if _, err = tx.ExecContext(ctx, "CREATE TABLE IF NOT EXISTS alembic_version (version_num VARCHAR(32) NOT NULL PRIMARY KEY)"); err != nil {
		return "", err
	}
	...
	if len(versions) > 1 {
		return "", fmt.Errorf("multiple database revisions are unsupported")
	}
	start := 0
	if len(versions) == 1 {
		start = -1
		for i, migration := range chain {
			if migration.Revision == versions[0] {
				start = i + 1
				break
			}
		}
		if start < 0 {
			return "", fmt.Errorf("unknown database revision %q; refusing migration", versions[0])
		}
	}
	for _, migration := range chain[start:] {
		if _, err = tx.ExecContext(ctx, migration.SQL); err != nil {
			return "", fmt.Errorf("migration %s: %w", migration.Revision, err)
		}
		if _, err = tx.ExecContext(ctx, "DELETE FROM alembic_version"); err != nil {
			return "", err
		}
		if _, err = tx.ExecContext(ctx, "INSERT INTO alembic_version (version_num) VALUES ($1)", migration.Revision); err != nil {
			return "", err
		}
	}
