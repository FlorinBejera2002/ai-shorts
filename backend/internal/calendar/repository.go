...
func (s *Repository) readPost(ctx context.Context, row rowScanner) (Post, error) {
	...
	var persistedInstagram map[string]any
	if e = json.Unmarshal(igJSON, &persistedInstagram); e != nil {
		return p, e
	}
	if len(persistedInstagram) > 0 {
		var igOptions publishing.InstagramOptions
		if e = json.Unmarshal(igJSON, &igOptions); e != nil {
			return p, e
		}
...
func encodeInstagramOptions(options publishing.InstagramOptions) []byte {
	if options == (publishing.InstagramOptions{}) {
		return []byte(`{}`)
	}
	encoded, _ := json.Marshal(options)
	return encoded
...
func (s *Repository) Mutate(ctx context.Context, userID, id string, input map[string]any, create bool) (Post, error) {
	...
	if !create && len(currentIGJSON) > 0 {
		if e = json.Unmarshal(currentIGJSON, &igOptions); e != nil {
			return empty, e
		}
	}
	...
	if status == "scheduled" {
		...
		if len(accounts) != len(accountIDs) {
			...
			for _, account := range accounts {
				...
				if !slices.Contains([]string{"instagram", "facebook", "tiktok", "youtube", "linkedin"}, account.provider) || !slices.Contains(platforms, account.provider) {
					issues = append(issues, Issue{"accountIds", "Only connected Instagram, Facebook, TikTok, YouTube and LinkedIn accounts can be scheduled"})
					break
				}
